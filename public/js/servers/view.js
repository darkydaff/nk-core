                    ...
                    <div class="panel p-4 space-y-4 relative group" id="client-card-${client.id}">
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-default/50">
                            ...
                            <div class="flex items-center gap-1">
                                <button onclick="window.serverView.clientAction(${client.id}, 'sync-stats', this)" class="p-2 text-muted hover:text-cyan-400 transition-all duration-200"><i class="fas fa-sync-alt"></i></button>
                                <button onclick="window.serverView.clientAction(${client.id}, '${isDisabled ? 'restore' : 'revoke'}', this)" class="p-2 ${isDisabled ? 'text-green-500 hover:text-green-400' : 'text-muted hover:text-orange-400'} transition-all duration-200"><i class="fas ${isDisabled ? 'fa-user-check' : 'fa-user-slash'}"></i></button>
                                <button onclick="window.serverView.clientAction(${client.id}, 'delete', this)" class="p-2 text-muted hover:text-red-500 transition-all duration-200"><i class="fas fa-trash-alt"></i></button>
                                <a href="/clients/${client.id}" class="ml-2 text-[10px] bg-primary text-white px-3 py-1.5 rounded-lg uppercase font-bold tracking-wider hover:bg-primary-hover transition-all">${this.labels.edit}</a>
                            </div>
                        </div>
                    </div>
                `;
            ...
            results.forEach(client => {
                ...
                html += `
                    <tr class="hover:bg-surface-hover/50 transition-colors group" id="client-row-${client.id}">
                        ...
                        <td class="px-5 py-4 text-right whitespace-nowrap cell-actions">
                            <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onclick="window.serverView.clientAction(${client.id}, 'sync-stats', this)" class="p-1.5 text-muted hover:text-cyan-400 transition-colors duration-200" title="${this.labels.sync}">
                                    <i class="fa-solid fa-rotate text-[10px]"></i>
                                </button>
                                <button onclick="window.serverView.clientAction(${client.id}, '${isDisabled ? 'restore' : 'revoke'}', this)" class="p-1.5 ${isDisabled ? 'text-green-500 hover:text-green-400' : 'text-muted hover:text-orange-400'} transition-colors duration-200" title="${isDisabled ? this.labels.restore : this.labels.revoke}">
                                    <i class="fas ${isDisabled ? 'fa-user-check' : 'fa-user-slash'} text-xs"></i>
                                </button>
                                <button onclick="window.serverView.clientAction(${client.id}, 'delete', this)" class="p-1.5 text-muted hover:text-red-500 transition-colors duration-200" title="${this.labels.delete}">
                                    <i class="fas fa-trash-alt text-xs"></i>
                                </button>
                                <a href="/clients/${client.id}" class="ml-2 text-[10px] bg-panel hover:bg-primary text-secondary hover:text-white px-2 py-1 rounded border border-default uppercase font-bold tracking-wider transition-all">
                                    ${this.labels.edit}
